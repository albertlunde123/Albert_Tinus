...
class lille_Puk():
    ...
    def __init__(self, path, m, R, t):
        ...
        self.t = t
        self.len = len(self.center[:, 0][self.t[0]:self.t[1]])

    def get_center(self, t):
        return self.center[:, t][self.t[0]:self.t[1]]

    def gc_err(self, t):
        err = []
        for i in self.center[:, t][self.t[0]:self.t[1]]:
            err.append(0.85*10**-2)
        return np.array(err)

    def get_edge(self, t):
        return self.edge[:, t][self.t[0]:self.t[1]]
