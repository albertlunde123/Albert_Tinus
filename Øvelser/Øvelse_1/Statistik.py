...
def error_function(x, f, popt, pcov):
    f_error = 0
    err = pcov # np.diagonal(popt)
...
error = error_function(xs, f, popt, pcov)

ax.plot(xs, f(xs, *popt))

ax.fill_between(xs, f(xs, *popt) + error, f(xs, *popt) - error, alpha = 0.3)
