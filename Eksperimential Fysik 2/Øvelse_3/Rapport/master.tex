...
L=n \cdot s
.\] 
Where L is the optical path length and s is the geometrical path length. If the object is filed with a gas the index of refraction is affected by the pressure of the gas, which then also affects the optical path length. 
\[
\Delta L = \Delta n \cdot s
.\] 
...
n = 1 + kp
.\] 
where k is a constant. The change in pressure is then related to the optical path length by.
\[
\Delta p = e
.\] 
\end{document}
