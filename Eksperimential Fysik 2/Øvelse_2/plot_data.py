...
def fit(V, *p):
    ...
    k = p[4]
    # e = p[4]
    return a*np.cos(b + V*d * np.exp((-k/800)*V)) + c
...
guess_params = [guess_a, -2.1, guess_c, guess_d, 1.03]
...
popt, pcov = scp.curve_fit(fit,
                    ...
                    voltage,
                    guess_params,
                    bounds = ((guess_a-1, -2.11, guess_c-1, guess_d - 0.1, 1.02), (guess_a+1, -2.09, guess_c+1, guess_d + 0.1, 1.04)))
